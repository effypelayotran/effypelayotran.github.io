...
// ----------------- dynamically reesize canvas -----------------
function resizeCanvas() {
	const canvas = document.getElementById("myCanvas");
	const homeHero = document.querySelector(".home-hero");

	let aspectRatio = simWidth / simHeight;
	let containerWidth = homeHero.clientWidth;
	let containerHeight = homeHero.clientHeight;
...
// Resize on window load and resize events
...
// Create a 'WebGlRenderingContext' object. The WebGLRenderingContext interface provides an interface to the OpenGL ES 2.0 graphics rendering context for the drawing surface of an HTML <canvas> element.
var gl = canvas.getContext("webgl");

/* Get tHE browser window's height and width, set that as the
	...
	updateCellColors() {
		...
		for (var i = 0; i < this.fNumCells; i++) {
			...
			if (this.cellType[i] == SOLID_CELL) {
				...
				// this.cellColor[3 * i] = 1.0; // White
				// this.cellColor[3 * i + 1] = 1.0; // White
				// this.cellColor[3 * i + 2] = 1.0; // White
			}
...
function setupScene() {
	...
	setObstacle(0.5, simHeight / 2.0, true);
	scene.paused = false;
...
// pass in the gl object we init that helps us interface the HTML and WebGL programs
...
function draw() {
	...
	if (scene.showGrid) {
		...
		let whiteColorArray = new Float32Array(scene.fluid.fNumCells * 3).fill(1.0);
		gl.bufferData(gl.ARRAY_BUFFER, whiteColorArray, gl.DYNAMIC_DRAW);
